...
The `TradeScenarioServlet` (URL: `/daytrader/scenario`) emulates a population of users by selecting a random trade operation on each HTTP request. The mix is defined in `TradeConfig.java` and determines what percentage of requests exercise each operation path.
