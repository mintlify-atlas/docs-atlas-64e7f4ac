---
title: "DayTrader 7 Application Architecture Overview"
...
- **`TradeSLSBBean`** — the primary EJB 3 `@Stateless` session bean implementation of `TradeServices`, backed by a JPA `EntityManager` and a `ManagedThreadFactory` for async order processing.
...
- **EJB3 mode**: `TradeAction` calls `TradeSLSBLocal.buy(userID, symbol, quantity, orderProcessingMode)` via the local EJB interface. Container-managed transactions are applied via `@TransactionAttribute(REQUIRED)`.
- **Direct mode**: `TradeAction` calls `TradeDirect.buy(...)` which uses JDBC directly against `jdbc/TradeDataSource`.

### TradeSLSBBean processes the order synchronously or asynchronously
`TradeSLSBBean.buy()` looks up the `AccountProfileDataBean` and `QuoteDataBean` via the JPA `EntityManager`, creates an `OrderDataBean` with status `"open"`, and debits the account balance.
...
After every buy or sell, `TradeAction` calls `updateQuotePriceVolume()`, which updates the `QuoteDataBean`'s `price`, `change1`, and `volume` fields, then calls `publishQuotePriceChange()` to send a `TextMessage` to `TradeStreamerTopic`.
...
| `TradeSLSBBean` | `...ejb3` | `@Stateless` EJB implementing `TradeServices` with JPA and JMS |
